from models.Level1GNN import Level1GNN
import torch
...
class WrapperLevel1GNN:
    ...
    def setLogger(self, logger):
        self.logger = logger
        self.logger.infoAll(inspect.getsource(Level1GNN))
        self.logger.infoAll(("Num Graph Layer: ", config.N_LAYER_LEVEL_1))
        self.logger.infoAll(("Layer TYPE:", config.LEVEL_1_LAYER))

    def resetModel(self, outSize):
        self.net = Level1GNN(outSize, layerType=config.LEVEL_1_LAYER, numNode=config.MAX_NODE + 1)
    ...
    def train(self, bioLoader5P):
        ...
        graphBatch, numGraph = nestedGraphData.createBatchForLevel(1)

        graphBatch2 = bioLoader5P.graphBatch

        print(len(graphBatch.batch), len(graphBatch2.batch))
        print(sum(graphBatch.batch), sum(graphBatch2.batch))
        assert torch.all(torch.eq(graphBatch.edge_index, graphBatch2.edge_index))
        assert torch.all(torch.eq(graphBatch.x, graphBatch2.x))
        assert torch.all(torch.eq(graphBatch.batch, graphBatch2.batch))
        ...
        for epoch in range(config.N_EPOCH):
            ...
            print("\r%s" % epoch, end="")
            optimizer.zero_grad()
            x = self.model.forward(graphBatch)
