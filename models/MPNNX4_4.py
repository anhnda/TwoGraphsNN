from models.net4_4 import Net4_4
import torch
...
class MPNNX4_4:
    def __init__(self):
        ...
        logPath = "%s/logs/MPNNX44_%s" % (config.C_DIR, utils.getCurrentTimeString())
        print("Logging path: ", logPath)
        self.logger = MyLogger(logPath)

        self.logger.infoAll(inspect.getsource(Net4_4.__init__))
        self.logger.infoAll(inspect.getsource(Net4_4.forward))
    ...
    def resetModel(self, numAtomFeature):
        self.net4 = Net4_4(numNode=config.MAX_NODE + 1, numAtomFeature=numAtomFeature)
