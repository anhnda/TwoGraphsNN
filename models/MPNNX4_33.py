...
class MPNNX4_33:
    def __init__(self):
        ...
        logPath = "%s/logs/MPNNX433_%s" % (config.C_DIR, utils.getCurrentTimeString())
        print("Logging path: ", logPath)
        self.logger = MyLogger(logPath)

        self.logger.infoAll(inspect.getsource(Net4_3.__init__))
        self.logger.infoAll(inspect.getsource(Net4_3.forward))
    ...
    def resetModel(self, numAtomFeature):
        self.net4 = Net4_3(numNode=config.MAX_NODE + 1, numAtomFeature=numAtomFeature)
    ...
    def fit(self, bioLoader):
        self.resetModel(numAtomFeature=bioLoader.N_ATOMFEATURE)
        self.train(bioLoader, debug=True)
        return self.model
    def getParams(self):
        return "MPNNX43"
