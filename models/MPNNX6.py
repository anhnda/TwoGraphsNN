...
class MPNNX6:
    def __init__(self):
        ...
        logPath = "%s/logs/MPNNX6_%s" % (config.C_DIR, utils.getCurrentTimeString())
        print("Logging path: ", logPath)
        self.logger = MyLogger(logPath)

        self.logger.infoAll(inspect.getsource(Net6.__init__))
        self.logger.infoAll(inspect.getsource(Net6.forward))
    ...
    def resetModel(self, numAtomFeature):
        self.net = Net6(numNode=config.MAX_NODE + 1, numAtomFeature=numAtomFeature)
    ...
    def train(self, bioLoader6, debug=True, pred=True):
        ...
        for epoch in range(config.N_EPOCH):
            ...
            print("\r%s" % epoch, end="")
            optimizer.zero_grad()
            drugE, seE, x = self.model.forward(bioLoader6.x,
                                               bioLoader6.drugGraphData.edge_index, bioLoader6.seGraphData.edge_index,
                                               bioLoader6.drugTrainNodeIds,
                                               bioLoader6.seNodeIds,
                                               bioLoader6.graphBatch,
                                               bioLoader6.nDrug
                                               )

            err, out = self.getMatDotLoss(drugE, seE, bioLoader6.trainOutMatrix)

            out2 = out.detach().numpy()
            target2 = bioLoader6.trainOutMatrix.numpy()
            ...
            if debug and epoch % 10 == 0:
                ...
                outTest = self.model.cal(x[bioLoader6.drugTestNodeIds], x[bioLoader6.seNodeIds])
                outTest = outTest.detach().numpy()

                targetTest = bioLoader6.testOutMatrix.numpy()
