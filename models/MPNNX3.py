...
class MPNNX3:
    def __init__(self):
        ...
        logPath = "%s/logs/MPNNX3_%s" % (config.C_DIR, utils.getCurrentTimeString())
        print("Logging path: ", logPath)
        self.logger = MyLogger(logPath)

        self.logger.infoAll(inspect.getsource(Net3.__init__))
        self.logger.infoAll(inspect.getsource(Net3.forward))
    ...
    def resetModel(self):
        self.net3 = Net3(numNode=config.MAX_NODE + 1)
    ...
    def train(self, bioLoader3, debug=True, pred=True):
        ...
        for epoch in range(config.N_EPOCH):
            ...
            print("\r%s" % epoch, end="")
            optimizer.zero_grad()
            drugE, seE, x = self.model.forward(bioLoader3.x,
                                               bioLoader3.drugGraphData.edge_index, bioLoader3.seGraphData.edge_index,
                                               bioLoader3.drugTrainNodeIds,
                                               bioLoader3.seNodeIds,
                                               bioLoader3.drugFeatures
                                               )

            err, out = self.getMatDotLoss(drugE, seE, bioLoader3.trainOutMatrix)

            out2 = out.detach().numpy()
            target2 = bioLoader3.trainOutMatrix.numpy()
            ...
            if debug and epoch % 10 == 0:
                ...
                outTest = self.model.cal(x[bioLoader3.drugTestNodeIds], x[bioLoader3.seNodeIds])
                outTest = outTest.detach().numpy()

                targetTest = bioLoader3.testOutMatrix.numpy()
