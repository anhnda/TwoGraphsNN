from models.net4_2 import Net4_2
import torch
...
class MPNNX4_2:
    def __init__(self):
        ...
        logPath = "%s/logs/MPNNX42_%s" % (config.C_DIR, utils.getCurrentTimeString())
        print("Logging path: ", logPath)
        self.logger = MyLogger(logPath)

        self.logger.infoAll(inspect.getsource(Net4_2.__init__))
        self.logger.infoAll(inspect.getsource(Net4_2.forward))
    ...
    def resetModel(self, numAtomFeature):
        self.net42 = Net4_2(numNode=config.MAX_NODE + 1, numAtomFeature=numAtomFeature)
        self.model = self.net42.to(self.device)
