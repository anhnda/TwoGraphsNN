from models.net4_p2 import Net4P2
import torch
...
class MPNNXP4:
    def __init__(self):
        ...
        logPath = "%s/logs/MPNNXP4_%s" % (config.C_DIR, utils.getCurrentTimeString())
        print("Logging path: ", logPath)
        self.logger = MyLogger(logPath)

        self.logger.infoAll(inspect.getsource(Net4P2))
    ...
    def resetModel(self, numAtomFeature):
        self.net4 = Net4P2(numNode=config.MAX_NODE + 1, numAtomFeature=numAtomFeature)
    ...
    def train(self, bioLoader4, debug=True, pred=True):
        ...
        for epoch in range(config.N_EPOCH):
            ...
            validateOutput = bioLoader4.validOutMatrix.numpy()
            ...
            if debug and epoch % 10 == 0:
                ...
                outValid = self.model.cal(x[bioLoader4.drugValidateNodeIds], x[bioLoader4.seNodeIds])
                ...
                self.logger.infoAll(("Val: AUC, AUPR: ", aucv, auprv))
                ...
                allEval.append([aucv, auprv, auc, aupr])
