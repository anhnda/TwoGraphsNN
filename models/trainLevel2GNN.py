from models.Level2GNN import Level2GNN
import torch
...
class WrapperLevel2GNN:
    ...
    def setLogger(self, logger):
        self.logger = logger
        self.logger.infoAll(inspect.getsource(Level2GNN))
        self.logger.infoAll(("Num Graph Layer: ", config.N_LAYER_LEVEL_2))
        self.logger.infoAll(("Layer TYPE:", config.LEVEL_2_LAYER))


    def resetModel(self, outSize):
        self.net = Level2GNN(outSize, layerType=config.LEVEL_2_LAYER, maxNode=config.MAX_NODE + 1)
    ...
    def train(self, bioLoader5P):
        ...
        graph = nestedGraphData.getGraphListAtLevel(2)[0]
        ...
        for epoch in range(config.N_EPOCH):
            ...
            print("\r%s" % epoch, end="")
            optimizer.zero_grad()
            x = self.model.forward(graph.x, graph.edge_index)
