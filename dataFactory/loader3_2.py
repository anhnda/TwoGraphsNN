...
class BioLoader3_2:
    ...
    def loadValidInchi(self):
        path = BioLoader3_2.getPathIFold(0)
        fin = open(path)
        self.validInchi = set()
        while True:
            line = fin.readline()
    ...
    def createTrainTestGraph(self, fPath, allTrain=False):
        ...
        for drugId in self.drugTrainIdList:
            ...
            for seId in seIds:
                drugse_edge_index.append([drugId + self.DRUG_OFFSET, seId + self.SE_OFFSET])
                # drugse_edge_index.append([seId + self.SE_OFFSET, drugId + self.DRUG_OFFSET])

        drugse_edge_index = np.asarray(drugse_edge_index)
        drugse_edge_index = torch.tensor(drugse_edge_index, dtype=torch.long).t().contiguous()
        ...
        self.drugseGraphData =Data(x=self.x, edge_index=drugse_edge_index)
    ...
    def getPathIFold(iFold):
        pathTrain = "%s%s" % (BioLoader3_2.getTrainPathPref(), iFold)
        return pathTrain


if __name__ == "__main__":
    bioloader2 = BioLoader3_2()

    iFold = 1
    path = BioLoader3_2.getPathIFold(iFold)
