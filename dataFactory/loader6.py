...
class BioLoader6:
    ...
    def __init__(self):
        ...
        self.loadValidProteins()

    def loadValidInchi(self):
        path = BioLoader6.getPathIFold(0)
        fin = open(path)
        self.validInchi = set()
        while True:
            line = fin.readline()
    ...
    def loadValidProteins(self):
        f = open(config.DRUGBANK_INCHI_PROTEIN)
        proteinCounts = dict()
        while True:
            line = f.readline()
            if line == "":
            ...
            for p in proteins:
                utils.add_dict_counter(proteinCounts, p)
        f.close()
        sortedList = utils.sort_dict(proteinCounts)
        print(sortedList)
        MIN_COUNT = 3
        self.validProteins = set()
        for k, v in proteinCounts.items():
            if v >= MIN_COUNT:
                self.validProteins.add(k)
    ...
    def loadDrug2Proteins(self):
        ...
        for proteins in proteinListList:
            for protein in proteins:
                if protein != "":
                    if protein in self.validProteins:
    ...
    def getPathIFold(iFold):
        pathTrain = "%s%s" % (BioLoader6.getTrainPathPref(), iFold)
        return pathTrain


if __name__ == "__main__":
    bioloader2 = BioLoader6()

    iFold = 1
    path = BioLoader6.getPathIFold(iFold)
