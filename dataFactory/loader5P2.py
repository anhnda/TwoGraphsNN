        ...
        if not config.PROTEIN_TEST:
            if not config.UN_DIRECTED:
                for srcId, targetId in self.drug_edge_index:
                    if targetId in nonTrainDrugNodeIds and \
                ...
                for srcId, targetId in self.drug_edge_index:
                    if targetId in nonTrainDrugNodeIds and self.PROTEIN_OFFSET <= srcId < self.PROTEIN_OFFSET + self.nProtein:
                        continue
                    elif srcId in nonTrainDrugNodeIds and self.PROTEIN_OFFSET <= targetId < self.PROTEIN_OFFSET + self.nProtein:
                        continue
                    else:
                        filtered_drug_edge_index.append([srcId, targetId])
            ...
            self.validateInpMat[:, config.CHEM_FINGERPRINT_SIZE:].fill(0)
