...
def loadRe():
    path = "%s/reGraph" % config.OUTPUT_DIR
    fin = open(path)
    modelNames = []
    AUCs = []
    stdAUCs = []
...
def plot():
    ...
    SELECTED_MODELS = ["2Graph-Inter", "2Graph-NoInter", "Outer+Inner Feature", "Outer", "Inner", "MLFF", "SCCA"]
    SELECTED_INDICES = []
    ...
    for i in range(2):
        ...
        if METRIC == 'AUC':
            ax.set_ylim(bottom=0.72, top=0.77)
        else:
            ax.set_ylim(bottom=0.40, top = 0.48)
        ...
        plt.savefig("%s/%s_PredGraph.png" % (config.FIG_DIR, METRIC))
        plt.savefig("%s/%s_PredGraph.eps" % (config.FIG_DIR, METRIC))
